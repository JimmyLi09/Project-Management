...
export default function StatsView() {
  ...
  const byStage = STAGES.map((s) => [`${s[2]} ${s[1]}`, projects.filter((p) => projStage(p) === s[0]).length] as const);
  ...
  return (
    <>
      <div className="kpi-grid four">
        <MiniKpi label="Projects 项目总数" value={String(projects.length)} />
        <MiniKpi label="Total points 总积分" value={String(totalPts)} />
        <MiniKpi label="Active 进行中" value={String(activeCount)} />
        <MiniKpi label="Overdue phases 逾期阶段" value={String(overdue)} color={overdue ? 'var(--danger)' : 'var(--success)'} />
      </div>

      <div className="grid-2col" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 20, alignItems: 'start' }}>
        <div className="panel clip">
          <div className="panel-head"><span className="panel-title">By PM 按负责人</span></div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <tbody>
              <tr><th style={th}>PM</th><th style={{ ...th, textAlign: 'right' }}>Projects</th><th style={{ ...th, textAlign: 'right' }}>Active</th><th style={{ ...th, textAlign: 'right' }}>Points 积分</th></tr>
              {rows.length === 0 && <tr><td style={{ ...cell, color: 'var(--text2)' }} colSpan={4}>暂无数据</td></tr>}
              {rows.map(([n, v]) => (
                <tr key={n}>
        ...
        <div className="panel clip">
          <div className="panel-head"><span className="panel-title">By stage 按阶段</span></div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <tbody>
              {byStage.map(([label, n]) => (
                <tr key={label}>
