/* ===== 07 报价审批 · quotation maths =====
   ...
   applies one discount to the sell total and adds GST on top — library sell
   prices are exclusive of GST. Every quotation needs PD / BD approval; one
...
export interface QuoteTotals {
  list: number;       // sell total before discount
  discount: number;
  subtotal: number;   // after discount, before GST
  gst: number;
...
export function quoteTotals(sections: QuoteSection[], discountPct: number, gstRate: number): QuoteTotals {
  const list = round2(sections.reduce((a, s) => a + s.list, 0));
  const cost = round2(sections.reduce((a, s) => a + s.cost, 0));
  const discount = round2(list * discountPct / 100);
  const subtotal = round2(list - discount);
  const gst = round2(subtotal * gstRate);
  return { list, discount, subtotal, gst, total: round2(subtotal + gst), cost, margin: subtotal > 0 ? (subtotal - cost) / subtotal : null };
}

/* What stops a quotation from being submitted, and what the approver should see. */
export function quoteChecks(sections: QuoteSection[], discountPct: number, t: QuoteTotals, marginFloor: number, reason: string): CostCheck[] {
  const out: CostCheck[] = [];
